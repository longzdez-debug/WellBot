  ...
  const server = createServer(async (req, res) => {
    try {
      ...
      if (requestPath === '/health' || requestPath === '/healthz') {
        json(res, 200, { status: 'ok', service: 'hunt-web' });
        return;
      }
      ...
      if (requestPath.startsWith('/api/')) {
        ...
        if (!user) {
          json(res, 403, { error: 'user_not_registered' });
          return;
        }

        if (requestPath === '/api/bootstrap' && req.method === 'GET') {
          const [links, ads, priceDrops, stats, statsByLink] = await Promise.all([
            db.getUserLinks(user.id),
            db.getDashboardAds(user.id, 50),
            db.getDashboardPriceDrops(user.id, 30),
            db.getDashboardStats(user.id),
            db.getUserAdsCount(user.id),
          ]);

          json(res, 200, {
            user: { id: user.id, telegramId: user.telegram_id, username: user.username },
            links,
            ads,
            priceDrops,
            stats,
        ...
        if (linkMatch && req.method === 'PATCH') {
          ...
          if (typeof body.is_active !== 'boolean') {
            json(res, 400, { error: 'is_active_required' });
            return;
          }
      ...
      if (requestPathFrom(req).startsWith('/api/')) {
