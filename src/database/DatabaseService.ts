...
export class DatabaseService {
  ...
  async deleteLink(linkId: number, userId?: number): Promise<boolean> {
    const query = userId == null ? 'DELETE FROM links WHERE id=$1' : 'DELETE FROM links WHERE id=$1 AND user_id=$2';
    const params = userId == null ? [linkId] : [linkId, userId];
    const r = await this.pool.query(query, params);
    return (r.rowCount || 0) > 0;
  }

  async setLinkActive(linkId: number, userId: number, isActive: boolean): Promise<boolean> {
    const r = await this.pool.query(
      'UPDATE links SET is_active=$1, error_count=CASE WHEN $1 THEN 0 ELSE error_count END WHERE id=$2 AND user_id=$3',
      [isActive, linkId, userId]
    );
    return (r.rowCount || 0) > 0;
  }
  ...
  async bulkCreateAds(linkId: number, ads: Ad[]): Promise<number> {
    ...
    const placeholders = rows.map((ad, i) => {
      const base = i * 11;
      values.push(linkId, ad.external_id, ad.title, ad.description || null, ad.price || null, ad.image_url || null, ad.ad_url, ad.location || null, ad.address || null, ad.published_at || null, ad.updated_at || null);
      return `($${base + 1},$${base + 2},$${base + 3},$${base + 4},$${base + 5},$${base + 6},$${base + 7},$${base + 8},$${base + 9},$${base + 10},$${base + 11})`;
    }).join(',');
    const result = await this.pool.query(`INSERT INTO ads (link_id,external_id,title,description,price,image_url,ad_url,location,address,published_at,updated_at) VALUES ${placeholders} ON CONFLICT (external_id,link_id) DO NOTHING`, values);
  ...
  async getExistingAdExternalIdsForLink(linkId: number, externalIds: string[]): Promise<Set<string>> {
    ...
    const r = await this.pool.query<{ external_id: string }>('SELECT external_id FROM ads WHERE link_id=$1 AND external_id=ANY($2::text[])', [linkId, externalIds]);
  ...
  async getUserAdsCount(userId: number): Promise<{ linkId: number; linkPlatform: string; count: number }[]> {
    const r = await this.pool.query('SELECT l.id as "linkId",l.platform as "linkPlatform",COUNT(a.id) as "count" FROM links l LEFT JOIN ads a ON a.link_id=l.id WHERE l.user_id=$1 GROUP BY l.id ORDER BY l.id', [userId]);
    return r.rows;
  }

  async getDashboardAds(userId: number, limit = 50): Promise<DashboardAd[]> {
    const safeLimit = Math.min(Math.max(Math.floor(limit), 1), 100);
    const r = await this.pool.query<DashboardAd>(
      `SELECT a.*, l.platform AS link_platform, l.url AS link_url
       FROM ads a JOIN links l ON l.id=a.link_id
       WHERE l.user_id=$1 ORDER BY COALESCE(a.published_at,a.created_at) DESC,a.id DESC LIMIT $2`, [userId, safeLimit]);
    return r.rows;
  }

  async getDashboardPriceDrops(userId: number, limit = 30): Promise<DashboardPriceDrop[]> {
    const safeLimit = Math.min(Math.max(Math.floor(limit), 1), 50);
    const r = await this.pool.query<DashboardPriceDrop>(
      `SELECT ph.id,ph.external_id,ph.old_price,ph.new_price,ph.price_change_percent,ph.created_at,
              a.title,a.image_url,a.ad_url,l.platform AS link_platform
       FROM price_history ph
       JOIN ads a ON a.id=ph.ad_id
       JOIN links l ON l.id=a.link_id
       WHERE ph.user_id=$1 ORDER BY ph.created_at DESC,ph.id DESC LIMIT $2`, [userId, safeLimit]);
    return r.rows;
  }

  async getDashboardStats(userId: number): Promise<{ activeLinks: number; totalLinks: number; adsToday: number; priceDropsToday: number }> {
    const r = await this.pool.query<{ active_links: string; total_links: string; ads_today: string; drops_today: string }>(
      `SELECT
        COUNT(*) FILTER (WHERE is_active) AS active_links,
        COUNT(*) AS total_links,
        (SELECT COUNT(*) FROM ads a JOIN links l ON l.id=a.link_id WHERE l.user_id=$1 AND a.created_at >= CURRENT_DATE) AS ads_today,
        (SELECT COUNT(*) FROM price_history ph WHERE ph.user_id=$1 AND ph.created_at >= CURRENT_DATE) AS drops_today
       FROM links WHERE user_id=$1`, [userId]);
    const row = r.rows[0];
    return { activeLinks: Number(row?.active_links || 0), totalLinks: Number(row?.total_links || 0), adsToday: Number(row?.ads_today || 0), priceDropsToday: Number(row?.drops_today || 0) };
  }

  async getLastPriceForAd(linkId: number, externalId: string): Promise<{ price: string; adId: number } | null> {
    const r = await this.pool.query('SELECT a.price,a.id as ad_id FROM ads a WHERE a.link_id=$1 AND a.external_id=$2 ORDER BY a.updated_at DESC NULLS LAST,a.id DESC LIMIT 1', [linkId, externalId]);
    return r.rows[0] || null;
  }
  ...
  async createPriceDropRecord(userId: number, adId: number, externalId: string, oldPrice: string, newPrice: string, changePercent: number): Promise<boolean> {
    try {
      const r = await this.pool.query(`INSERT INTO price_history (ad_id,user_id,external_id,old_price,new_price,price_change_percent,notified_at) VALUES ($1,$2,$3,$4,$5,$6,CURRENT_TIMESTAMP) ON CONFLICT (user_id,external_id,old_price,new_price) DO NOTHING`, [adId, userId, externalId, oldPrice, newPrice, changePercent]);
      return (r.rowCount || 0) > 0;
    } catch (error: unknown) {
