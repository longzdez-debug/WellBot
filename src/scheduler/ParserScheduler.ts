...
export class ParserScheduler {
  ...
  constructor(db: DatabaseService, bot: BotHandler) {
    this.db = db;
    this.bot = bot;
    const s = parseInt(process.env.PARSE_INTERVAL_SECONDS || '5', 10);
    const c = parseInt(process.env.PARSE_CONCURRENCY || '5', 10);
    this.intervalMs = Math.max(1000, Number.isFinite(s) ? s * 1000 : 5000);
    this.concurrency = Math.max(1, Math.min(20, Number.isFinite(c) ? c : 5));
  ...
  async runParsing(): Promise<void> {
    ...
    try {
      ...
      const unique = links.filter(l => {
        const k = `${l.user_id}|${l.url}`;
        if (seen.has(k)) return false;
        seen.add(k);
        return true;
      });
      logger.info('🔄 Parsing cycle started', { linksCount: unique.length, concurrency: this.concurrency });
      const results = await this.mapWithConcurrency(unique, this.concurrency, l => this.parseLink(l));
      const allNew: Array<{ad:any;telegramId:number}> = [];
      const allDrops: Array<{drop:any;telegramId:number;userId:number}> = [];
      const userIds = [...new Set(unique.map(l => l.user_id))];
      ...
      for (const [id,u] of entries) if (u) users.set(id, u);
      for (let i=0; i<unique.length; i++) {
        const r = results[i], u = users.get(unique[i].user_id);
        if (!r || !u) continue;
        for (const ad of r.newAds) allNew.push({ ad, telegramId: u.telegram_id });
        for (const drop of r.priceDrops) allDrops.push({ drop, telegramId: u.telegram_id, userId: u.id });
      }
      await Promise.all([this.notifyNewAds(allNew), this.notifyPriceDrops(allDrops)]);
      logger.info('Parsing cycle completed', { duration: `${Date.now()-start}ms`, linksCount: unique.length, totalNewAds: allNew.length });
    } catch (error:any) {
  ...
  private async mapWithConcurrency<T,R>(items:T[], limit:number, worker:(item:T)=>Promise<R>):Promise<R[]> {
    ...
    const run = async () => {
      while (true) {
        ...
        catch (error:any) { logger.error('Scheduler worker failed', { index:i, error:error?.message }); }
      }
    };
  ...
  private async notifyNewAds(items:Array<{ad:any;telegramId:number}>):Promise<void> {
    ...
    for (const {ad,telegramId} of items) {
      const g = groups.get(telegramId) ?? [];
      if (!g.some(x => x.external_id === ad.external_id)) g.push(ad);
      groups.set(telegramId,g);
    }
  ...
  private async notifyPriceDrops(items:Array<{drop:any;telegramId:number;userId:number}>):Promise<void> {
    ...
    for (const item of items) {
      const g = groups.get(item.telegramId) ?? [];
      if (!g.some(x => x.drop.externalId === item.drop.externalId)) g.push({drop:item.drop,userId:item.userId});
      groups.set(item.telegramId,g);
    }
    await Promise.all([...groups].map(async ([telegramId,drops]) => {
      const userId = drops[0]?.userId;
      if (!userId) return;
      const sub = await this.db.getActiveChannelSubscription(userId);
      await Promise.all(drops.map(async ({drop}) => {
        ...
        if (sub) try { await this.bot.sendPriceDropNotification(sub.channel_id,drop); }
        catch (error:any) { logger.error('Failed to send price drop to channel',{channelId:sub.channel_id,externalId:drop.externalId,error:error.message}); }
      }));
    }));
  }

  private async parseLink(link:ParseLink):Promise<ParseResult> {
    const newAds:any[] = [], priceDrops:any[] = [];
    try {
      ...
      try {
        ads = await Promise.race([
          parser.parseUrl(link.url),
          new Promise<never>((_,reject) => setTimeout(() => reject(new Error('Parse timeout after 9 seconds')),9000))
        ]);
      } catch (error:any) {
      ...
      const externalIds = [...new Set(ads.filter(a => a?.external_id).map(a => String(a.external_id)))];
      if (baseline) {
        ...
        logger.info('Baseline snapshot stored; no notifications sent',{linkId:link.id,adsCount:created.size});
        return {newAds,priceDrops};
      }
      ...
      for (const adData of ads) {
        ...
        if (existing.has(id)) {
          ...
          if (last) await this.processPriceDrop(last,id,adData.price,priceDrops,link.id);
          continue;
        }
      ...
      if (current && current.error_count >= 5) {
        await this.db.markLinkInactive(link.id);
        logger.warn('Link marked inactive',{linkId:link.id,errorCount:current.error_count});
      }
      return {newAds,priceDrops};
    }
  }

  private async processPriceDrop(last:{price:string;adId:number},externalId:string,newPrice:string,priceDrops:any[],linkId:number):Promise<void> {
    try {
      const oldN = this.db.parsePriceToNumber(last.price), newN = this.db.parsePriceToNumber(newPrice);
      if (oldN === null || newN === null) return;
      if (newN < oldN) {
        const pct = ((oldN-newN)/oldN*100).toFixed(1);
        const recorded = await this.db.createPriceDropRecord(last.adId,externalId,last.price,newPrice,parseFloat(pct));
        if (recorded) priceDrops.push({adId:last.adId,oldPrice:last.price,newPrice,changePercent:pct,externalId,linkId});
      }
      if (oldN !== newN) await this.db.updateAdPrice(last.adId,newPrice);
