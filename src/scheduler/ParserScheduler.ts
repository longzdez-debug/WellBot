...
export class ParserScheduler {
  ...
  async runParsing(): Promise<void> {
    ...
    try {
      ...
      logger.info('Parsing cycle completed', {
        ...
        linksCount: unique.length,
        totalNewAds: allNew.length,
        totalPriceDrops: allDrops.length,
      });
    } catch (error: unknown) {
  ...
  private async notifyPriceDrops(items: Array<{ drop: PriceDrop; telegramId: number; userId: number }>): Promise<void> {
    ...
    await Promise.all([...groups].map(async ([telegramId, drops]) => {
      ...
      for (const { drop } of drops) {
        ...
        if (subscription) {
          try {
            await this.bot.sendPriceDropNotification(subscription.channel_id, drop);
          } catch (error: unknown) {
            const message = error instanceof Error ? error.message : String(error);
  ...
  private async processPriceDrop(
    last: { price: string; adId: number },
    externalId: string,
    newPrice: string,
    priceDrops: PriceDrop[],
    linkId: number,
