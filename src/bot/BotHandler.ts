...
export class BotHandler {
  ...
  async handleConfirmAddLink(chatId: number, userId: number): Promise<void> { try { let url = this.pendingLinks.get(userId); if (!url) { await this.bot.sendMessage(chatId, '❌ Ссылка не найдена. Попробуйте отправить её снова.'); return; } url = this.normalizeUrl(url); const assessment = LinkAcceptance.assess(url); if (!assessment.ok || !assessment.platform) { await this.bot.sendMessage(chatId, '❌ Ошибка валидации ссылки.'); this.pendingLinks.delete(userId); return; } await this.db.createUser(userId, null); const user = await this.db.getUser(userId); if (!user?.id) { await this.bot.sendMessage(chatId, '❌ Ошибка: пользователь не найден.'); this.pendingLinks.delete(userId); return; } if (await this.db.getUserLinksCount(user.id) >= 10) { await this.bot.sendMessage(chatId, '⚠️ Достигнут лимит в 10 ссылок.'); this.pendingLinks.delete(userId); return; } const existingLinks = await this.db.getUserLinks(user.id); if (existingLinks.some(link => link.url === url)) { await this.bot.sendMessage(chatId, '⚠️ Эта ссылка уже добавлена.'); this.pendingLinks.delete(userId); return; } await this.db.createLink(user.id, url, assessment.platform); this.pendingLinks.delete(userId); await this.bot.sendMessage(chatId, '✅ Ссылка добавлена! Вы будете получать уведомления о новых объявлениях.', { reply_markup: this.getMainKeyboard() }); if (this.scheduler) { logger.info('Triggering immediate parse after direct link confirmation', { userId, url }); this.scheduler.triggerParse(); } logger.info('Link confirmed and added', { userId, platform: assessment.platform, url }); } catch (error: any) { logger.error('Failed to confirm add link', { userId, error: error.message }); await this.bot.sendMessage(chatId, '❌ Не удалось добавить ссылку.'); this.pendingLinks.delete(userId); } }
